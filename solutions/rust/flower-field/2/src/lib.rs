    ...
    for i in 0..height {
        ...
        for j in 0..width{
            ...
            if *current_row[j] == b'*'{
                ...
                if j > 0 && prev_row.get(j-1).is_some() && *prev_row[j-1] == b'*' {
                    flower_count += 1;
                }
                if prev_row.get(j).is_some() && *prev_row[j] == b'*' {
                    flower_count += 1;
                }
                if prev_row.get(j+1).is_some() && *prev_row[j+1] == b'*' {
                    flower_count += 1;
                }

                if j > 0 && current_row.get(j-1).is_some() && *current_row[j-1] == b'*' {
                    flower_count += 1;
                }
                if current_row.get(j+1).is_some() && *current_row[j+1] == b'*' {
                    flower_count += 1;
                }

                if j > 0 && next_row.get(j-1).is_some() && *next_row[j-1] == b'*' {
                    flower_count += 1;
                }
                if next_row.get(j).is_some() && *next_row[j] == b'*' {
                    flower_count += 1;
                }
                if next_row.get(j+1).is_some() && *next_row[j+1] == b'*' {
                    flower_count += 1;
                }
